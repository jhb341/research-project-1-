...
from gradcam_spharm import GradCam, visualize_1d_heatmap
...
def step(model, train_loader, device, criterion, epoch, logger, nclass, optimizer=None, pbar=False):
    ...
    logger.write([epoch + 1, running_loss.item() / iter, accuracy, *torch.mean(total_dice, 0).tolist(), torch.mean(total_dice[:, 1:]).item()])
...
def main(args):
    ...
    # try to get class names from label.csv for logging
    lut_path = os.path.join(args.data_dir, "label.csv")
    class_names = []

    if (os.path.isfile(lut_path)):
        lut = { "0": "None" }
        with open(lut_path) as lut_csv:
            lines = lut_csv.readlines()[1:]
            for line in lines:
                className, classID = line.strip().split(',')
                className = className.split(' ')[1]
                lut[classID] = className
        class_names = [f"Dice {lut[i]:10}" for i in map(str, args.classes)]
    else:
        class_names = [f"Dice {i:10}" for i in map(str, args.classes)]
    ...
    for partition_type in partition:
        ...
        logger[partition_type].write(
            ["{:10}".format("Epoch"), "{:15}".format("Loss"), "{:15}".format("Accuracy"), *class_names, "{:15}".format("Average Dice")]
        )
    ...
    # hook
    target_layer = model.final.shconv[1]
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        #input_tensor = features.to(device)            # [B, C, N]
        logits = model(input_tensor)                   # [B, num_classes, N]
        cls_scores = logits.mean(dim=2)                # [B, num_classes]
        pred_class = cls_scores.argmax(dim=1).item()  # 스칼라
        cam = grad_cam.generate_cam(input_tensor[0:1], target_class=pred_class)
