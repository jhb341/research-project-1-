...
class GradCam(nn.Module):
    ...
    def generate_cam(self, input_tensor: torch.Tensor, target_class: int = None) -> np.ndarray:
        ...
        pooled_grads = torch.mean(grads, dim=2, keepdim=True)
...
"""
   for 1D heatmap visualization with SPHARM-Net 
"""
def visualize_1d_heatmap(cam: np.ndarray,
                         figsize: tuple = (8, 1),
                         cmap: str = 'jet',
                         xlabel: str = 'Index',
                         title: str = None) -> None:
    """
    1D CAM activation을 heatmap 형태로 시각화합니다.

    Parameters:
        cam     : 1D numpy array (N,)
        figsize : (width, height) 인치
        cmap    : matplotlib 컬러맵 이름
        xlabel  : x축 레이블
        title   : 그래프 타이틀 (None이면 표시 안 함)
    """
    fig, ax = plt.subplots(figsize=figsize)
    im = ax.imshow(cam[np.newaxis, :], cmap=cmap, aspect='auto')
    ax.set_yticks([])
